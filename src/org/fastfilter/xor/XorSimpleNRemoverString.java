...
public class XorSimpleNRemoverString implements StringFilter {
     ...
     */
    public static XorSimpleNRemoverString construct(Set<String> keysIn, Set<String> keysOut, int bitsPerFingerprint, int tSize) {
        return new XorSimpleNRemoverString (keysIn, keysOut, bitsPerFingerprint, tSize);
    }
     ...
     * @return a new Filter with the selected parameters
     */
    public static XorSimpleNRemoverString construct(Set<String> keysIn, Set<String> keysOut, int bitsPerFingerprint) {
        return new XorSimpleNRemoverString (keysIn, keysOut, bitsPerFingerprint);
    }    
     ...
     * @return a new Filter with the selected parameters
     */ 
    XorSimpleNRemoverString(Set<String> keysIn, Set<String> keysOut, int bitsPerFingerprint) {
     ...
     * @return a new Filter with the selected parameters
     */
    XorSimpleNRemoverString(Set<String> keysIn, Set<String> keysOut, int bitsPerFingerprint, int tSize) {
        ...
        while (true) {
            ...
            long[] stack = new long[(keysIn.size()+keysOut.size()) * 2];
            // Set with the hashes that correspond to the positive element hashes
            Set<Long> hs = new HashSet<Long>();
            // Try to create a map for each s of S to a position i and add it to the stack
            if (map(keysIn, keysOut, hf, stack, hs)) {
                ...
                assign(stack, data, hs);
                return;
            }
            //Abort if it is not able to converge in 10 iterations
            if (tries%10==0) {
            	System.out.println("+10 iterations completed");//TODO: Make it configurable
            	return;
            }
        }
    }
	 ...
	 */    
    boolean map(Set<String> keysIn, Set<String> keysOut, HashFunction hf, long[] stack, Set<Long> hashes) {
        ...
        for (String k : keysIn) {
            ...
            // Identify which hashed should be positive
            hashes.add(x); //TODO: optimize speed by replacing it with the approach used in the non string implementation
        }
        // For elements in F do the same process without adding the hashes to "hashes" element
     ...
     * @param hs The set of hashes that correspond to positives
     */
    void assign(long[] stack, long[] b, Set<Long> hs) {
    	// Traverse the stack
        for(int stackPos = stack.length; stackPos > 0;) {
            ...
            long stored = hs.contains(x)?fingerprint(x):(fingerprint(x)^mask);
    ...
    int h(long x, int index) {
    	...
    	//return reduce((int) Long.rotateLeft(x, index * 21), blockLength) + index * blockLength;
    }
	...
	/**
	 * Assign the hash to 1 of the possible n positions
	 * @param hash element to be assigned
	 * @param n block size, number of positions
	 * @return position assigned
	 */
    public int reduce(int hash, int n) {
        return ((hash & 0x1fffff)%n);
    }
