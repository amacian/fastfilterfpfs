 ...
 *
 */
public class XorIntegratedFilterTByFile implements Filter {
	...
	/** Optimizing the calculation of power of 2s */
	long[] pows = null;
	/**
     ...
     */
    public static XorIntegratedFilterTByFile construct(Set<Long> keysIn, LineNumberReader keysOutFile, int outSize, int bitsN, int bitsF) {
        return new XorIntegratedFilterTByFile (keysIn, keysOutFile, outSize, bitsN, bitsF);
    }    
     ...
     * @param bitsF Number of 1-bit subfilters
     */
    XorIntegratedFilterTByFile(Set<Long> keysIn, LineNumberReader keysOutFile, int outSize, int bitsN, int bitsF) {
    	// The total number of bits is the sum of bits from the first filter and
    	// the bits used for the equivalent 1-bit subfilters.
    	//long in1 = System.nanoTime();
    	...
    	int inSize = keysIn.size();
    	int size = inSize + ((bitsF==1)?(int) Math.ceil(outSize/Math.pow(2, bitsN)):0);
        ...
        Set<Long>[] bitAssignationIn = new HashSet[bitsF];
        Set<Long>[] bitAssignationOut = new HashSet[bitsF];
        
        // Create the sets for the f subfilters
        for (int i=0; i<bitsF;i++) {
        	bitAssignationIn[i] = new HashSet<Long>();
        	//bitAssignationIn.add(new HashSet<Long>());
        	//bitAssignationOut.add(new HashSet<Long>());
        	bitAssignationOut[i] = new HashSet<Long>();
        }
        ...
        while (!success) {
	        ...
	        while (true) {
	            ...
	            if (map(keysIn, null, seed, stack, inSize, 0)) {
			...
			for (int i=0; i<bitsF;i++) {
				...
				Set<Long> ins = bitAssignationIn[i];//bitAssignationIn.get(i);
				// Retrieve the elements from F belonging to that subfilter
				Set<Long> outs = bitAssignationOut[i];//bitAssignationOut.get(i);
		        /* Populating last bitsF from the filter */
	            // Stack that will hold the pair element hash, selected position
				int subSize = ins.size();
				int subOutSize = outs.size();
				long[] stack = new long[(subSize+subOutSize) * 3];
	            // Try to create a map for each s of S, and t of F 
				// to a position i and add it to the stack
    	        
				if (map(ins, outs, seed, stack, subSize, subOutSize)) {
	            	// If it was possible to create the map, assign value to the
     ...
     * @param x Hash of an element from S or F
     * @return subfilter that corresponds to x
     */
	int c(long x) {
		return (bitsF==1)?0:(int)Math.abs((x>>32)%bitsF);
	 ...
	 * @param inSize size to be considered for input keys
	 * @param outSize size to be considered for output keys
	 * @return true if the map could be done, false otherwise.
	 */
    boolean map(Set<Long> keysIn, Set<Long> keysOut, long seed, long[] stack, int inSize, int outSize) {
        ...
        int[] Cout = null;
        long[] Hout = null; 
        // Only to be used whenever there are elements to be excluded
        if (outSize>0) {
        ...
        while (si < 3 * size) {
            ...
            if ((Ctot[i]) == 1) {
            	...
            	boolean in = (C[i]==1);
            	// Get the 64-bit hash from that position in the appropriate array
    ...
    void assign(long[] stack, long[] b, long mask, boolean filter) {
    	...
    	for(int stackPos = stack.length; stackPos > 0;) {
    		// Check if the element is from S or F
    ...
    public boolean mayContain(long key) {
    	...
    	// pows was initialized during creation
    	long mask = maskN | pows[c(x)];
     ...
     * @return if it gives a positive
     */
    boolean mayContain(long key, long mask) {
    	// Create the 64-bit hash
    	long x = Hash.hash64(key, seed);
